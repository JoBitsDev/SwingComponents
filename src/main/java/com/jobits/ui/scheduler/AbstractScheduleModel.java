...
public abstract class AbstractScheduleModel implements ScheduleModel {
    ...
    @Override
    public void addAppointmentChangeListener(AppointmentChangeListener listener) {
        _listenerList.add(AppointmentChangeListener.class, listener);
    }

    @Override
    public void removeAppointmentChangeListener(AppointmentChangeListener listener) {
     ...
     * @param index The location to add the resource, or -1 to add it at the
     * end.
     */
    protected void fireResourceAdded(Resource resource, LocalDate dateTime, int index) {
    ...
    /**
     * Protected method which an implementation of the Schedule Model should
     * call every time a resource should be removed from the model.
     *
     * @param resource (not null) Resource being removed
     * @param dateTime (not null) Date the resource is being removed
     */
    protected void fireResourceRemoved(Resource resource, LocalDate dateTime) {
        // Guaranteed to return a non-null array
        Object[] listeners = _listenerList.getListenerList();
        // Process the listeners last to first, notifying
        // those that are interested in this event
    ...
    /**
     * Protected method which an implementation of the Schedule Model should
     * call every time a resource should be updated. Like the title or color has
     * changed.
     *
     * @param resource (not null) Resource being updated
     */
    protected void fireResourceUpdated(Resource resource) {
        // Guaranteed to return a non-null array
        Object[] listeners = _listenerList.getListenerList();
        // Process the listeners last to first, notifying
        // those that are interested in this event
     ...
     */
    protected void fireAppointmentAdded(Appointment appointment) {
        // Guaranteed to return a non-null array
        Object[] listeners = _listenerList.getListenerList();
        // Process the listeners last to first, notifying
        // those that are interested in this event
    ...
    /**
     * Protected method called whenever the model has removed an appointment
     * from a day.
     *
     * @param appointment (not null) The appointment being removed.
     */
    protected void fireAppointmentRemoved(Appointment appointment) {
        // Guaranteed to return a non-null array
        Object[] listeners = _listenerList.getListenerList();
        // Process the listeners last to first, notifying
        // those that are interested in this event
    ...
    /**
     * Protected method called whenever the model has updated some details about
     * an appointment, like the title, time or resource it is assigned to.
     *
     * @param appointment (not null) The appointment.
     */
    protected void fireAppointmentUpdated(Appointment appointment) {
        // Guaranteed to return a non-null array
        Object[] listeners = _listenerList.getListenerList();
        // Process the listeners last to first, notifying
        // those that are interested in this event
